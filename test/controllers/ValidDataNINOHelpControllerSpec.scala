...
import models.{NormalMode, SelectAlternativeService, SelectNINOLetterAddress, UserAnswers, ValidDataNINOHelp}
...
lazy val ValidDataNINOHelpRoute = routes.ValidDataNINOHelpController.onPageLoad(mode = NormalMode).url

val formProvider = new ValidDataNINOHelpFormProvider()
val form = formProvider()


"InvaltaNINOHelp Controller" - {
  ...
  "must return OK and the correct view for a GET" in {
    ...
    running(application) {
      val request = FakeRequest(GET, ValidDataNINOHelpRoute)
  ...
  "must populate the view correctly on a GET when the question has previously been answered" in {
    ...
    running(application) {
      ...
      status(result) mustEqual OK
      contentAsString(result) contains view(form.fill(ValidDataNINOHelp.values.head), NormalMode)(request, messages(application), config).toString
    }
  }
  ...
  "must redirect to the next page when valid data is submitted" in {
    ...
    running(application) {
      val request =
        FakeRequest(POST, ValidDataNINOHelpRoute)
          .withFormUrlEncodedBody(("value", ValidDataNINOHelp.values.head.toString))
