  ...
  "EnteredPostCodeNotFound Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(EnteredPostCodeNotFound.values.head), NormalMode)(request, messages(application)).toString
      }
    }
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, enteredPostCodeNotFoundRoute)
            .withFormUrlEncodedBody(("value", EnteredPostCodeNotFound.values.head.toString))
