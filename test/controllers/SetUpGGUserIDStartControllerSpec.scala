...
class SetUpGGUserIDStartControllerSpec extends SpecBase {

  "SetUpGGUserIDStart Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, routes.SetUpGGUserIDStartController.onPageLoad().url)

        val result = route(application, request).value

        val view = application.injector.instanceOf[SetUpGGUserIDStartView]
