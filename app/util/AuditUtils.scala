...
object AuditUtils {
  ...
  def buildAuditEvent(
    personDetails: Option[PersonalDetails] = None,
    ...
    personDetails match {
      case Some(pd) =>
        ...
        buildDataEvent(
          ...
          Json.toJson(
            buildDetails(
              if (usePostcode) Some(pd.postCode.getOrElse("")) else None,
  ...
  def buildBasicEvent(auditType: String)(implicit hc: HeaderCarrier): ExtendedDataEvent     = {
    val strPath = hc.otherHeaders.toMap.get("path")
    ExtendedDataEvent(
      auditSource = auditSource,
      auditType = auditType,
