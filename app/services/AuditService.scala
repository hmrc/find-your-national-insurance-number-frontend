...
class AuditService @Inject() (auditConnector: AuditConnector)(implicit val ec: ExecutionContext) extends Logging {
  ...
  def findYourNinoPDVMatchFailed(pdvData: PDVResponseData, origin: Option[OriginType])(implicit
    ...
    audit(
      AuditUtils.buildAuditEvent(
        ...
        identifierType = StringUtils.EMPTY,
        origin = origin
      )
    )

  def findYourNinoPDVNoMatchData(origin: Option[OriginType])(implicit headerCarrier: HeaderCarrier): Unit =
    audit(
      AuditUtils.buildAuditEvent(
        personDetails = None,
        auditType = "FindYourNinoPDVMatchFailed",
  ...
  def findYourNinoGetPdvDataHttpError(status: String, reason: String, origin: Option[OriginType])(implicit
    hc: HeaderCarrier
  ): Unit =
    audit(
      AuditUtils.buildAuditEvent(
        auditType = "FindYourNinoError",
        validationOutcome = StringUtils.EMPTY,
        identifierType = StringUtils.EMPTY,
        pageErrorGeneratedFrom = Some("/checkDetails"),
  ...
  def findYourNinoPDVMatched(pdvData: PDVResponseData, origin: Option[OriginType], idData: Option[IndividualDetails])(
    ...
    audit(
      AuditUtils.buildAuditEvent(
        ...
        validationOutcome = pdvData.validationStatus.toString,
        identifierType = idData.map(_.crnIndicator.asString).getOrElse(StringUtils.EMPTY),
        origin = origin
      )
    )

  def findYourNinoOptionChosen(pdvData: Option[PDVResponseData], optionChosen: String, origin: Option[OriginType])(
    implicit hc: HeaderCarrier
  ) =
    audit(
      AuditUtils.buildAuditEvent(
        ...
        validationOutcome = pdvData.map(_.validationStatus).getOrElse(ValidationStatus.Failure).toString,
  ...
  def findYourNinoConfirmPostcode(
    ...
    audit(
      AuditUtils.buildAuditEvent(
        ...
        validationOutcome = pdvData.map(_.validationStatus).getOrElse(ValidationStatus.Failure).toString,
        identifierType = pdvData.map(_.CRN.getOrElse(StringUtils.EMPTY)).getOrElse(StringUtils.EMPTY),
  ...
  def findYourNinoOnlineLetterOption(
    pdvData: Option[PDVResponseData],
    idAddress: Address,
    value: String,
    origin: Option[OriginType]
